...
# - search_terms: ["Your Job Title 1 remote", "Your Job Title 2 remote"]
# - min_salary: 100000 (your minimum)
# - target_titles: ["director", "manager"] (leadership levels you want)
# - it_subspecialties: ["platform", "cloud"] (your domain keywords)
# - negative_keywords: ["helpdesk", "support"] (roles to avoid)
# - local_search_enabled: True/False (enable Pass 2 for hybrid/on-site roles)
# - commutable_areas: ["Your City, ST", ...] (towns within commute distance)
...
```bash
# Run complete scrape + score pipeline in Claude Code
/job-scraper

# Then create materials for a role you want to pursue
/job-apply
```

## What Each Skill Does

- **`/job-scraper`** - Full pipeline: scrape → enhance → filter → score. Start here.
- **`/job-apply`** - Create materials for a specific role already identified as worth pursuing
- **`/job-materials`** - Generate resume + cover letter for a specific job posting
